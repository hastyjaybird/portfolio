...
- **`acepr`** — **[ACTION]** decide whether the newly-surfaced Parallel 18 advisor role gets its own experience folder or stays folded into ACEPR's README.
...
- **`iogs`**, **`skysource-xprize`**, **`eklutna-10power`**, **`side-projects`**, **`volunteer-work`**, **`resilient-markets-maxeta`** — not yet reached in the review loop; check each README's own "Still needed" section once reviewed, and this file will get a proper entry for each at that point.
